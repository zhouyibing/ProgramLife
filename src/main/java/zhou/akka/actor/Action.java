...
import zhou.akka.actor.Interface.InterfaceCreator;

public class Action extends UntypedActor{
	ActorRef actorA= getContext().actorOf(Props.create(new InterfaceCreator("InterfaceA")));
	ActorRef actorB= getContext().actorOf(Props.create(new InterfaceCreator("InterfaceB")));
	ActorRef actorC= getContext().actorOf(Props.create(new InterfaceCreator("InterfaceC")));
		public Action() {
			super();
		}
